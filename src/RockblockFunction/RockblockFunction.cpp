...
Table *add_sensor_data(Table *t, uint64_t time, float ATH30_temperature, float ATH30_humidity, float BMP390_temperature, float BMP390_pressure) {
    t = checkTable(t);
    add_entry(t, (TableEntry){ .time = time, .ATH30_temperature = ATH30_temperature, .ATH30_humidity = ATH30_humidity, .BMP390_temperature = BMP390_temperature, .BMP390_pressure = BMP390_pressure });
    return t;
}
